...
\end{align}
let $p \sim Bern\,(\pr{X\leq x})$ such that   
\begin{align}
\pr{p=1} &= \pr{X\leq x} = F(x) \\
\pr{p=0} &= 1-\pr{X\leq x} = 1-F(x)
\end{align}
Let $P$ follows Binomial distribution taking $n$ independent trails from above Bernoulli's distribution    
\begin{align}
\pr{P=a} &= \comb{n}{a}\,\pr{p=1}^{a}\,\pr{p=0}^{n-a} \\
\pr{P=a} &= \comb{n}{a}\,F(x)^{a}\,(1-F(x))^{n-a}\label{bin}  
\end{align}
